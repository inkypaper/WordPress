...
$possible_plugin_locations = array(
	...
	'C:\inetpub\wwwroot\wp-content\plugins\wp-optimize-premium\cache',
);
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\inetpub\wwwroot\wp-content\plugins\wp-optimize-premium\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-site.test.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-site.test.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-site.test.php');
}
